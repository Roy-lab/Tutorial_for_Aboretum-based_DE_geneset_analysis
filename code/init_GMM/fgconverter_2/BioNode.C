...
int
BioNode::standardizeData_LogSpace()
{
	calculateMean_LogSpace();	
	calculateStdev_LogSpace(mean);	
	double mean=getMean();
	double std=getStdev();
	for(int g=0;g<expLevels.size();g++)
	{
...
BioNode::zeroMeanData_LogSpace()
{
	...
	for(int g=0;g<expLevels.size();g++)
	{
		...
		//expLevels[g]=exp(expLevels[g]-mean);
		//expLevels[g]=expLevels[g]-mean;
		if(isnan(expLevels[g]))
		{
			cout <<"Stop! Found nan for " << name << endl;
		}
...
int
BioNode::calculateMean_LogSpace()
{
	mean=0;
	for(int i=0;i<expLevels.size();i++)
	{
		mean=mean+log(expLevels[i]+1);
		//mean=mean+expLevels[i];
	}
...
int
BioNode::calculateStdev_LogSpace(double mean)
{
	stdev=0;
	for(int i=0;i<expLevels.size();i++)
	{
