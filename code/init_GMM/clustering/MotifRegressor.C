...
MotifRegressor::learnMoE_CrossValidation(int neededfoldCnt)
{
	...
	for(int i=0;i<foldCnt;i++)
	{
		...
		trainLL[i]=currScore;
		holdoutLL[i]=testLL;
		double newunpenscore=getUnpenalScore();
		holdout_unpenLL[i]=newunpenscore;
		if(i<foldCnt-1)
		{
			cleanUp();
		}
	...
	for(map<int,double>::iterator tIter=holdoutLL.begin();tIter!=holdoutLL.end();tIter++)
	{
		...
		double unpenalizedLL = holdout_unpenLL[tIter->first];
		VSET& varSet=varMgr->getVariableSet();	//JSmod
		double varsz = varSet.size();		//JSmod, #_genes
		double dim=evMgr->getNumberOfEvidences();	//JSmod, #_columns
		double penaltyBIC = (double) ((double) dim*expertCnt) *((double) 1/2)*log(varsz);
		double penaltyAIC = (double) ((double) dim*expertCnt) *((double) 1/2)*2;
		double LL_correctedBIC = unpenalizedLL - penaltyBIC;
		double LL_correctedAIC = unpenalizedLL - penaltyAIC;
		cout << "#_gene=" << varsz << "\t#_column=" << dim << "\t#_clusters=" << expertCnt << endl;
		cout << "penaltyBIC=" << penaltyBIC <<"\tpenaltyAIC=" << penaltyAIC << endl;
		llFile << holdout_unpenLL[tIter->first] <<"\t"<< trainLL[tIter->first] <<"\t" << LL_correctedBIC <<"\t"<< LL_correctedAIC << endl;
	}
	llFile.close();
	return 0;
}
...
MotifRegressor::getScore()
{
	...
	//VSET& varSet=varMgr->getVariableSet();	//JSmod
	//int dim=evMgr->getNumberOfEvidences();	//JSmod
	//cout <<"#_gene :" << varSet.size() << "\t#_exp :" << dim << "\t#_cluster :" << expertCnt <<endl;	//JSmod
...
//JS modified
double 
MotifRegressor::getUnpenalScore()
{
	double outputScore=0;
	for(map<int,Matrix*>::iterator dIter=exprProfileSet.begin();dIter!=exprProfileSet.end();dIter++)
	{
